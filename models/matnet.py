...
def get_model_groupdata(group_data, mask, is_training, bn_decay=None):
    ...
    with tf.variable_scope('edge_net1') as sc:
        net, kernel,_,_ = edge_net.edge_unit(input_image, mask, 'max', config.neighbor_num,32, scope='conv1', bn=True, is_training=is_training,
                                bn_decay=bn_decay)
...
def get_model_mask(point_cloud, index,mask, is_training, bn_decay=None):
    ...
    group_data = tf_grouping.group_ma_tensor(point_cloud, index, batch_size, num_point)
    group_data = tf.reshape(group_data, [batch_size, num_point, neighbor_num * 4])  # B N K*4
    with tf.variable_scope('ma_net') as sc:
        net_only_ma, transform_only_ma = get_model_point(point_cloud,is_training, bn_decay=None)
    end_points['transform_ma'] = transform_only_ma
    with tf.variable_scope('group_data_net') as sc:
        net_only_groupdata, transform_groupdata = get_model_groupdata(group_data,mask,is_training, bn_decay=None)
    end_points['transform_groupdata'] = transform_groupdata
    with tf.variable_scope('ec_net') as sc:
        net_only_ec, transform_ec = get_model_ec(group_data,mask,is_training, bn_decay=None)
